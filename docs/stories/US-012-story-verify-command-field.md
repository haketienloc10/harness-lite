...
- Migration `002-story-verify.sql` adds `verify_command`, `last_verified_at`,
  and `last_verified_result` to `story`.
...
- Commands: `scripts/bin/harness-cli story add --verify`,
  `scripts/bin/harness-cli story update --verify`.
- Tables: `story`.
