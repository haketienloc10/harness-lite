...
Harness can verify every story with a configured `verify_command` in one command
and report passed, failed, and skipped stories.
...
- `cargo test --workspace` passed with
  `story_verify_all_reports_pass_fail_and_skipped`.
- Final proof: `scripts/bin/harness-cli story verify-all` passed with 21 stories
  checked, 11 passed, 0 failed, and 10 skipped without `verify_command`.
