...
Merged pull requests should leave a project-visible changelog entry. If a merged
PR changes the Rust Harness CLI tool or its release packaging, the repo should
publish a fresh CLI release for downstream installers.
...
- `ruby -e 'require "yaml"; ARGV.each { |f| YAML.load_file(f); puts "ok #{f}" }' .github/workflows/harness-cli-release.yml .github/workflows/post-merge-maintenance.yml`:
  passed.
...
- `bash -n scripts/install-harness.sh && bash -n scripts/build-harness-cli-release.sh`:
  passed.
- `actionlint`: not installed locally, so GitHub-specific workflow linting was
  not run.
- 2026-06-09 follow-up: GitHub run `27180707313` failed in
  ...
  `printf --` for all bullet formats. Local reproduction of the PR #13 changelog
  entry passed.
